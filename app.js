...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize section editing handlers
    setupSectionEditingHandlers();
});
...
function setupEventListeners() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('edit-section-btn')) {
            ...
            openSectionEditor(sectionContent, sectionName);
        }
    });
    });
...
function setupSectionEditingHandlers() {
    // Save section content button
    const saveContentBtn = document.getElementById('save-content-btn');
    if (saveContentBtn) {
        saveContentBtn.addEventListener('click', saveSectionContent);
    }
}

function openSectionEditor(sectionElement, sectionName) {
    if (!checkAdminStatus()) {
        showAlert('Admin access required', 'error');
        return;
    }
    ...
    if (textarea) {
        // Get current content excluding edit buttons
        const clone = sectionElement.cloneNode(true);
        clone.querySelectorAll('.edit-section-btn').forEach(btn => btn.remove());
...
function saveSectionContent() {
    if (!currentEditingSection || !checkAdminStatus()) {
        return;
    }
    ...
    if (textarea) {
        ...
        // Update section content and re-add edit button
        currentEditingSection.element.innerHTML = newContent + 
            '<button class="btn-glass edit-section-btn admin-only" data-section="' + 
            currentEditingSection.name + '" style="display:inline-block;">Edit This Section</button>';
        
        // Update admin UI to show/hide elements
        updateAdminUI();
        
        closeContentEditor();
        showAlert('✓ Section updated successfully', 'success');
        
        // Save brief content (in memory)
        saveBriefToStorage();
    }
}

function saveBriefToStorage() {
    // Update current brief object with latest content
    currentBrief.content = document.querySelector('.brief-container').innerHTML;
    currentBrief.lastModified = new Date().toISOString();
    
    // In a real application, this would save to a backend
    // For this demo, we maintain state in memory
}
...
function setupAdminEventHandlers() {
    ...
    // Content editor save button is now handled in setupSectionEditingHandlers
